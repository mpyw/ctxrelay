// Package gotaskchecker checks that gotask tasks properly derive context
...
const gotaskPkgPrefix = "github.com/siketyan/gotask"
...
func (c *Checker) checkDoFamilyCalls(cctx *checkers.CheckContext, call *ast.CallExpr) {
...
func (c *Checker) checkDoAsyncCall(cctx *checkers.CheckContext, call *ast.CallExpr) {
	sel, ok := call.Fun.(*ast.SelectorExpr)
	if !ok {
		return
	}
	...
	pkg := named.Obj().Pkg()
	if pkg == nil || !strings.HasPrefix(pkg.Path(), gotaskPkgPrefix) {
		return
	}
...
func isGotaskPackageCall(pass *analysis.Pass, sel *ast.SelectorExpr) bool {
	...
	return strings.HasPrefix(pkgName.Imported().Path(), gotaskPkgPrefix)
}

// exprContainsDeriver checks if an expression contains a call to the deriver function.
func (c *Checker) exprContainsDeriver(cctx *checkers.CheckContext, expr ast.Expr) bool {
	switch e := expr.(type) {
	case *ast.FuncLit:
		return c.matcher.SatisfiesAnyGroup(cctx.Pass, e.Body)
...
func (c *Checker) identContainsDeriver(cctx *checkers.CheckContext, ident *ast.Ident) bool {
	...
	funcLit := checkers.FindFuncLitAssignmentBefore(cctx, v, ident.Pos())
	if funcLit != nil {
		return c.matcher.SatisfiesAnyGroup(cctx.Pass, funcLit.Body)
	}
...
func (c *Checker) callExprContainsDeriver(cctx *checkers.CheckContext, call *ast.CallExpr) bool {
	...
	return c.matcher.SatisfiesAnyGroup(cctx.Pass, call)
}
...
func (c *Checker) traceCallToFuncLitReturn(cctx *checkers.CheckContext, call *ast.CallExpr) bool {
	...
	funcLit := checkers.FindFuncLitAssignmentBefore(cctx, v, call.Pos())
	if funcLit == nil {
		return false
	}
...
func (c *Checker) funcLitReturnContainsDeriver(cctx *checkers.CheckContext, funcLit *ast.FuncLit) bool {
	var found bool
...
func findCallExprAssignmentBefore(cctx *checkers.CheckContext, v *types.Var, beforePos token.Pos) *ast.CallExpr {
...
func findCallExprInAssignment(cctx *checkers.CheckContext, assign *ast.AssignStmt, v *types.Var) *ast.CallExpr {
