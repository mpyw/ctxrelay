// Package goroutinederivechecker checks that goroutines call specified functions
// to derive a new context (e.g., apm.NewGoroutine).
package goroutinederivechecker
...
func (c *Checker) CheckGoStmt(cctx *checkers.CheckContext, goStmt *ast.GoStmt) {
	...
	if !c.callSatisfiesDerive(cctx, call) {
		cctx.Reportf(goStmt.Pos(), "goroutine should call %s to derive context", c.matcher.Original)
	}
}
...
func (c *Checker) callSatisfiesDerive(cctx *checkers.CheckContext, call *ast.CallExpr) bool {
	switch fun := call.Fun.(type) {
	case *ast.FuncLit:
		// go func() { ... }()
		// If the func literal has its own context parameter, it will be checked in its own scope
		if checkers.HasContextOrCarrierParam(cctx.Pass, fun.Type, cctx.Carriers) {
			return true
		}
		return c.matcher.SatisfiesAnyGroup(cctx.Pass, fun.Body)
...
func (c *Checker) identFuncSatisfiesDerive(cctx *checkers.CheckContext, ident *ast.Ident) bool {
	...
	if checkers.HasContextOrCarrierParam(cctx.Pass, funcLit.Type, cctx.Carriers) {
		return true
	}
...
func (c *Checker) returnedFuncSatisfiesDerive(cctx *checkers.CheckContext, call *ast.CallExpr) bool {
...
func (c *Checker) funcLitReturnSatisfiesDerive(cctx *checkers.CheckContext, funcLit *ast.FuncLit) bool {
	var satisfies bool
...
func (c *Checker) returnedValueSatisfiesDerive(cctx *checkers.CheckContext, result ast.Expr) bool {
	if innerFuncLit, ok := result.(*ast.FuncLit); ok {
		if checkers.HasContextOrCarrierParam(cctx.Pass, innerFuncLit.Type, cctx.Carriers) {
			return true
		}
		return c.matcher.SatisfiesAnyGroup(cctx.Pass, innerFuncLit.Body)
	}
